...
CONNECTION_OPENERS = frozenset({"db.py", "api/deps.py", "api/launch.py"})
...
地基）：建库 + 起服务，所以它开连接。
（`gate.py` 曾经是第三个：M2「防泄漏」考试的独立入口，随秘密下线一起删了。）
...
def test_allowlist_stays_small() -> None:
    ...
    # gate.py 曾经也在这儿（M2 考试入口），随秘密下线一起删了（ADR 0039）——
    # **名单变短是好事**：每一个成员都是「时态过滤只写一次」可以被绕开的一个地方。
...
# ══════════════════════════════════════════════════════════════════════════
# 花作者的钱的那几个入口：**系统自动的那条只有一处，作者亲手按的那条只有一处**
# ══════════════════════════════════════════════════════════════════════════

SUMMARY_BUYERS = ["api/app.py", "api/background_runtime.py"]
"""能真正调 `RollingSummarizer.ensure()` 的文件，**排好序，一个不多**。

- `api/background_runtime.py` —— 系统自动那条。两个触发（保存之后 / 每 30 分钟扫描）
  不是两条执行路：它们都只是**下一张单**，单最后都汇到 `chapter_refresh` 的协调器，
  由这个 adapter 去付那一次钱。
- `api/app.py` —— **作者亲手按**那条（`POST …/summary` ← 右栏那颗「重新生成」）。
"""


def test_only_one_place_per_trigger_kind_can_buy_a_chapter_summary() -> None:
    """**能真正调 `RollingSummarizer.ensure()` 的地方，每一类触发只许有一处。**

    ── 它守的是什么 ────────────────────────────────────────────────────────

    不是「入口越少越好」，是**每一个入口都得有人回答那三个问题**。这条守卫的全部
    作用就是逼着加入口的人回答一次，然后把答案写在这儿。

    ── 名单上这两处各自的答案（2026-09-05）──────────────────────────────

    `api/background_runtime.py`（系统自动）：
      · 谁按的？没人按 —— 所以它**不许**碰撤回过的章
        （`chapter_refresh._head_missing`：他删一次系统买回来一次 = 花他没按过的钱
        抹掉他刚做的动作），也不许碰他正盯着的那一章（焦点防抖）。
      · 他知道自己在付费吗？不知道，所以那一侧「不该 422」，失败要装进回执
        （`deps.model_configuration_error` 写着这条）。

    `api/app.py`（作者亲手按「重新生成」）：
      · 谁按的？他。所以上面那两条约束都不适用 —— 花的是他授权的钱。
      · 他知道自己在付费吗？知道：按钮上方那句话写着「跑一次模型」，
        且模型没配好时 `get_summarizer` 当场 422 顶回去。
      · 撤回过的章会不会被它买回来？**会，而那正是它存在的理由。**
        2026-08-25 这条路删过一次，代价当天没被写下来：系统按纪律不补、作者又没入口，
        于是撤回过的章**永远**拿不回机器总结。2026-09-05 维护者裁定加回来
        （原话：「撤回了之后肯定这个底部要留一个重新生成的按钮供用户立即生成」）。

    ── 它红了代表什么 ──────────────────────────────────────────────────────

    有人加了第三个付费入口。这条产品线上最容易犯的错就是这个：这个仓库修过一次
    「一条零调用方的付费入口」（ADR 0035，换章 autopilot），修过一次「防抖在执行层
    根本没生效，改一下午存三十次就买三十次」。**多一个入口 = 多一处要重新论证
    「这笔钱是谁按的」**——论证完把答案加到上面那张名单里，别只改断言。
    """
    import ast
    ...
    assert offenders == SUMMARY_BUYERS, (
        f"能买一份章节总结的地方变了：{offenders}，名单上是 {SUMMARY_BUYERS}。"
        "每一类触发只许一处，而每一处都得回答「这笔钱是谁按的、他知不知道、"
        "撤回过的章会不会被它买回来」——答案写在 SUMMARY_BUYERS 和本函数的 docstring 里。"
    )
