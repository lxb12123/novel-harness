...
def test_anything_that_is_not_the_engines_own_mark_is_left_alone(what: str, text: str) -> None:
    ...
    这六种都长得很像自述，但没有一种是**引擎自己发明的那个记号**：
    `【自述】` / `（自述）` 是中文里天生就有的括号加一个常用词，认它就等于宣布
    「作者的正文里不许出现这几个字」——而误切的方向是**正文进自述、然后被丢掉**，
    那是作者的稿子。留着的代价只是他的稿子开头多一行机器话：看得见，能自己删。

    所以这一条是对**未来那个更聪明的修法**的守卫：谁把这里改成「像自述就认」，
    这六条一起红。
    """
    body, note = split_self_note(text)
