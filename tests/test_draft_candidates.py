...
def test_the_writer_closes_the_mark_like_a_bracket_and_the_half_lands_on_the_card() -> None:
    """**2026-08-13 在作者的真书上实测：12 稿里有 2 稿的自述末尾多了一个 `〗`。**

    模型把 `〖自述〗` 当成一对括号用了——开头照抄了整个记号，收尾又补上右半边：
    `〖自述〗……节奏轻快。〗`。**切到行尾那一刀没错**（那半个括号确实没进作者的书），
    错在切下来的那一段自己还挂着记号的碎屑，而这一段是原样摆到作者选版本那张卡上的：
    作者看见的是一个乱码字，看不出那是引擎自己的记号漏了半边。
    """
    body, note = split_self_note(
        f"{SELF_NOTE_MARK}这一版侧重姜召作为父亲寻找家人的温情与重逢的喜悦，"
        "略去战斗描写，节奏轻快。〗\n\n姜召推开门。"
    )
    assert note.endswith("节奏轻快。"), f"自述末尾挂着记号的碎屑：{note!r}"
    assert body == "姜召推开门。", "正文那一侧一个字都不许动"


@pytest.mark.parametrize(
    ("what", "text", "want_note", "want_body"),
    [
        (
            "收尾补了右半边（实测那一档）",
            f"{SELF_NOTE_MARK}这一版更冷。〗\n\n风雪落在肩上。",
            "这一版更冷。",
            "风雪落在肩上。",
        ),
        (
        ...
        (
            "整个记号在同一行里写了两遍（把自述前后包起来）",
            f"{SELF_NOTE_MARK}这一版更冷。{SELF_NOTE_MARK}\n\n风雪落在肩上。",
            "这一版更冷。",
            "风雪落在肩上。",
        ),
        (
            "续写那一次在行中又补了半个（两段是直接拼起来的）",
            f"{SELF_NOTE_MARK}这一版更冷。〗\n\n风雪落在肩上。{SELF_NOTE_MARK}接着写。〗\n\n他抬起头。",
            "这一版更冷。",
            "风雪落在肩上。\n\n他抬起头。",
        ),
...
def test_no_piece_of_the_mark_ever_reaches_the_card(
    what: str, text: str, want_note: str, want_body: str
) -> None:
    """**记号的碎屑（整记号、落单的半边括号）不属于任何人的散文，一律不上屏。**

    判据只在字符层面：碎屑的定义**跟着 `SELF_NOTE_MARK` 自己走**（整个记号，
    加上它那两个半边），不去猜模型想表达什么（ADR 0005）。

    削的只有**自述那一侧**——它已经从正文里切出来了，再削一刀只动屏幕上那张卡，
    作者的稿子一个字都碰不到。正文那一侧反过来：见下面那条「不认」的清单。
    """
    body, note = split_self_note(text)
    assert note == want_note, f"{what}：自述是 {note!r}"
    assert body == want_body, f"{what}：正文被动过了"


def test_a_bracket_pair_inside_the_note_is_prose_not_debris() -> None:
    """**成对的括号一律不动**，只削「配不上对」的那半个。

    模型真的会在自述里写「删掉了〖回忆〗那一段」——那对括号是它的散文。
    见谁削谁的话，作者读到的是一句缺了字的话，而他没有第二份可以对照。
    """
    body, note = split_self_note(
        f"{SELF_NOTE_MARK}这一版删掉了〖回忆〗那一段。〗\n\n风雪落在肩上。"
    )
    assert note == "这一版删掉了〖回忆〗那一段。"
    assert body == "风雪落在肩上。"

    # 落单的那半边在句子**中间**：削不动它（削了就是从句子当中拿走一个字），
    # 而它也不可能是记号掉的——记号只会掉在切口那两端。
    _, mid = split_self_note(f"{SELF_NOTE_MARK}这一版删掉了〖回忆那一段。\n\n风雪落在肩上。")
    assert mid == "这一版删掉了〖回忆那一段。"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("记号里多了一个空格", "〖自述 〗这一版更冷。\n\n风雪落在肩上。"),
        ("换了个说法", "〖说明〗这一版更冷。\n\n风雪落在肩上。"),
        ("别的括号族，但在行中", "风雪落在肩上。【自述】这一版更冷。\n\n他抬起头。"),
        ("别的括号族，但在后面的行", "风雪落在肩上。\n\n（自述）他终于抬起头。"),
    ],
)
def test_anything_that_is_not_the_engines_own_mark_is_left_alone(what: str, text: str) -> None:
    """**认不出就一个字都不动**（`split_self_note` 的立场，这条不许松）。

    这六种都长得很像自述，但没有一种是**引擎自己发明的那个记号**摆在它该在的位置：
    `【自述】` / `（自述）` 是中文里天生就有的括号加一个常用词，在行中或后面的行认它
    就等于宣布「正文里不许出现这几个字」——而误切的方向是**正文进自述、然后被丢掉**，
    那是作者的稿子。

    **唯一的例外在下一条**：第一个非空行的开头（ADR 0048 之后稿子直接进书，那儿漏一次
    就是作者的书里多一行机器话）。所以这一条仍是对**「像自述就认」**那种修法的守卫：
    谁把行中 / 后面的行也认了，这六条一起红。
    """
    body, note = split_self_note(text)
    assert (body, note) == (text, ""), f"{what}：引擎认了一个不是它自己的记号"
...
@pytest.mark.parametrize(
    ...
    [
        ("方括号", "【自述】这一版更冷。\n\n风雪落在肩上。", "这一版更冷。"),
        ("直角引号（真书上 2026-09-12 那一次）", "「自述」以贾环视角贯穿。\n\n夜雨初歇。", "以贾环视角贯穿。"),
        ("圆括号", "（自述）这一版更冷。\n\n风雪落在肩上。", "这一版更冷。"),
        ("带冒号", "【自述】：这一版更冷。\n\n风雪落在肩上。", "这一版更冷。"),
        ("前面有空行", "\n「自述」这一版更冷。\n\n风雪落在肩上。", "这一版更冷。"),
    ],
)
def test_a_lookalike_mark_opening_the_first_line_is_cut(what: str, text: str, note: str) -> None:
    """**第一个非空行以记号变体开头 ⇒ 切下来当自述**（ADR 0048 之后唯一放宽的一处）。

    2026-09-12 真书上写手把 `〖自述〗` 写成了 `「自述」`，那一行
    「「自述」以贾环视角贯穿……」直接进了作者的第 158 章——起草即写入之后，
    「看得见，能自己删」不再成立。第一行开头是我们要它写自述的位置，写手的散文不会
    从那儿起笔，切它不可能切到正文。
    """
    body, got = split_self_note(text)
    assert got == note, what
    assert "自述" not in body, f"{what}：那一行机器话还在正文里"
    assert body == text.split("\n\n", 1)[1] if text.startswith(("【", "「", "（")) else True
