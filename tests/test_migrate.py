...
def test_migrate_twice_is_idempotent(tmp_path: Path) -> None:
    ...
    assert migrate(c) == 39
    assert migrate(c) == 39  # 不抛
    assert user_version(c) == 39
    c.close()
...
def test_migrate_twice_on_fresh_connections(tmp_path: Path) -> None:
    ...
    c1.close()
    c2 = connect(path)
    assert migrate(c2) == 39
    assert user_version(c2) == 39
    c2.close()
...
def test_migration_files_are_readable_from_package() -> None:
    ...
    assert names == [
        ...
        "029_character_information.sql",
        "030_project_language.sql",
        "031_r2_ruleset_epoch.sql",
        "032_import_toc_skip.sql",
        "033_notification_title_code.sql",
        "034_event_cast_notification.sql",
        "035_r3_ruleset_epoch.sql",
        "036_edge_start_only.sql",
            "037_draft_candidate_brief.sql",
            "038_ruleset_state_for_bootstrapped_books.sql",
            "039_coverage_retry_index.sql",
    ]
    ...
    assert "PRAGMA user_version = 4" in (
        root / "004_chapter_summary.sql"
    ).read_text(encoding="utf-8")
    assert "PRAGMA user_version = 5" in (
        root / "005_fact_edit.sql"
    ).read_text(encoding="utf-8")
    assert "PRAGMA user_version = 6" in (
        root / "006_chat_session.sql"
    ).read_text(encoding="utf-8")
    assert "PRAGMA user_version = 7" in (
        root / "007_draft_candidate.sql"
    ).read_text(encoding="utf-8")
    assert "PRAGMA user_version = 8" in (
        root / "008_cache_usage.sql"
    ).read_text(encoding="utf-8")
...
def test_populated_v1_database_migrates_without_changing_existing_rows(tmp_path: Path) -> None:
    ...
    # `project` 030 之前从没在迁移里加过列（`node` 也没有），所以这两个历来都用
    # `SELECT *`。030 是第一个给 project 加列的迁移——同 `before_proposal` 已经踩过
    # 的坑一样，加了列之后 `SELECT *` 就不再是「v1 那几列」了，改成明写列名，
    # 新增的两列另外单独断言默认值（见下面的 `added_project`）。
    before_project = tuple(
        c.execute(
            "SELECT id, name, root_path, canon_version, created_at, updated_at "
    ...
    added_project = c.execute(
        "SELECT language, language_locked FROM project WHERE id = ?", (project_id,)
    ).fetchone()
    assert tuple(added_project) == ("zh", 0), "030 之前落库的项目该落在 DEFAULT 上，不是被判定过"
...
def test_concurrent_first_migrate_does_not_race(tmp_path: Path) -> None:
    ...
    assert results == [39] * n, f"并发首跑必须全部成功，实得 {results}"
    c = connect(path)
    assert user_version(c) == 39
...
def test_v16_upgrade_plants_ruleset_baseline_and_synthetic_refresh_runs(
    ...
    """v16 旧书升级后：每个项目恰有一行 epoch=`CURRENT_RULESET_EPOCH` 的 ruleset state
    （hash == 目录常量），每章恰有一条 generation=1 的 synthetic refresh run；重跑迁移不新增。

    ⚠️ **2026-09-05：epoch 又 +1，现在是 3**（迁移 035 删 R3，ADR 0042）。
    ⚠️ **2026-08-27：epoch 从 1 变 2**。018 先给这本 v16 旧书种下 epoch=1、
    hash=`SYSTEM_RULESET_V1_HASH` 的基线；新迁移 031（删 R2，[ADR 0040]
    (../docs/adr/0040-future-leak-cut.md)）紧接着在同一趟 `migrate()` 里把它
    （连同所有既有项目）递增到 epoch=2、hash=`CURRENT_RULESET_HASH`。这条测试量的是
    「v16 旧书迁到今天最新」的最终结果，不是 018 单独那一步——018 那一步单独的
    冻结字面量断言在下面 `test_migration_018_frozen_hash_matches_the_catalog_constant`。
    """
    ...
    assert ruleset[0]["epoch"] == CURRENT_RULESET_EPOCH
    assert ruleset[0]["ruleset_hash"] == CURRENT_RULESET_HASH
    ...
    assert migrate(conn) == 39  # 幂等：不新增第二行
    assert (
        conn.execute(
...
def test_migration_038_plants_the_baseline_for_books_born_without_one(tmp_path: Path) -> None:
    """从浏览器 / 桌面版「新建 / 导入」建出来的书（`onboarding.bootstrap_project()` →
    `project.insert()`）2026-09-13 之前没有 `validation_ruleset_state` 行——「分析本章」500、
    保存后的整理和 30 分钟扫描静默跳过。038 给这些书补上当前 epoch/hash 的一行；
    已经有行的一行不多；重跑不新增。"""
    from importlib.resources import files

    conn = connect(tmp_path / "born-bare.db")
    try:
        # 先逐脚本迁到 037，再按当年的坏法造一本没有基线的书（裸 INSERT，绕开今天会补行的 insert()）
        root = files("novel_harness") / "migrations"
        for name in sorted(p.name for p in root.iterdir() if p.name.endswith(".sql")):
            if int(name[:3]) > 37:
                break
            conn.executescript((root / name).read_text(encoding="utf-8"))
        assert user_version(conn) == 37
        conn.execute(
            "INSERT INTO project (id, name, root_path) VALUES (?, ?, ?)",
            ("project:born-bare", "裸书", "books/裸书"),
        )
        conn.execute(
            "INSERT INTO project (id, name, root_path) VALUES (?, ?, ?)",
            ("project:has-one", "有行的", "books/有行的"),
        )
        conn.execute(
            "INSERT INTO validation_ruleset_state (project_id, epoch, ruleset_hash) VALUES (?, 2, 'old')",
            ("project:has-one",),
        )
        conn.commit()
        ...
        assert migrate(conn) == 39
        rows = {
            r["project_id"]: (r["epoch"], r["ruleset_hash"])
            for r in conn.execute(
                "SELECT project_id, epoch, ruleset_hash FROM validation_ruleset_state"
            ).fetchall()
        }
        assert rows["project:born-bare"] == (CURRENT_RULESET_EPOCH, CURRENT_RULESET_HASH)
        assert rows["project:has-one"] == (2, "old"), "已有的行不动"
        assert migrate(conn) == 39
        assert (
            conn.execute("SELECT count(*) FROM validation_ruleset_state").fetchone()[0] == 2
        )
    finally:
        conn.close()

    sql = (
        files("novel_harness") / "migrations" / "038_ruleset_state_for_bootstrapped_books.sql"
    ).read_text(encoding="utf-8")
    assert f"SELECT id, {CURRENT_RULESET_EPOCH}, '{CURRENT_RULESET_HASH}'" in sql
