...
def test_migrate_twice_is_idempotent(tmp_path: Path) -> None:
    ...
    assert migrate(c) == 20
    assert migrate(c) == 20  # 不抛
    assert user_version(c) == 20
    c.close()
...
def test_migrate_twice_on_fresh_connections(tmp_path: Path) -> None:
    ...
    c1.close()
    c2 = connect(path)
    assert migrate(c2) == 20
    assert user_version(c2) == 20
    c2.close()
...
def test_migration_files_are_readable_from_package() -> None:
    ...
    assert names == [
        ...
        "016_rule_until.sql",
        "017_chapter_refresh.sql",
        "018_summary_versions.sql",
        "019_canon_edge_overrides.sql",
        "020_extraction_superseded.sql",
    ]
...
def test_concurrent_first_migrate_does_not_race(tmp_path: Path) -> None:
    ...
    assert results == [20] * n, f"并发首跑必须全部成功，实得 {results}"
    c = connect(path)
    assert user_version(c) == 20
...
def test_v16_upgrade_plants_ruleset_baseline_and_synthetic_refresh_runs(
    ...
    assert migrate(conn) == 20  # 幂等：不新增第二行
    assert (
        conn.execute(
