...
def test_migrate_twice_is_idempotent(tmp_path: Path) -> None:
    ...
    assert migrate(c) == 33
    assert migrate(c) == 33  # 不抛
    assert user_version(c) == 33
    c.close()
...
def test_migrate_twice_on_fresh_connections(tmp_path: Path) -> None:
    ...
    c1.close()
    c2 = connect(path)
    assert migrate(c2) == 33
    assert user_version(c2) == 33
    c2.close()
...
def test_concurrent_first_migrate_does_not_race(tmp_path: Path) -> None:
    ...
    assert results == [33] * n, f"并发首跑必须全部成功，实得 {results}"
    c = connect(path)
    assert user_version(c) == 33
...
def test_v16_upgrade_plants_ruleset_baseline_and_synthetic_refresh_runs(
    ...
    assert migrate(conn) == 33  # 幂等：不新增第二行
    assert (
        conn.execute(
