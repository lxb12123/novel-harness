...
def hydrate_proposal_names(
    ...
    ── 为什么这件事必须在后端做 ─────────────────────────────────────────
    在这个函数存在之前，界面拿 `subject_id` / `target_id` 去**角色册**里查名字，
    查不到就把 id 截断了摆上屏（`n:ID22`）。**缝在两次查询的时间差**：角色册和提案
    队列在浏览器里是两条独立缓存，而后台抽取 / 自动升 CANON 会造出新节点——没有任何
    一条路径保证角色册那份在提案那份之后重取过。（角色册本身不挑 label：它走
    `resolve(pid, None)`，而每个节点建出来就带一条 canonical 别名。）

    出参自足 ⇒ 这一整类失败在结构上不存在，也不必再给一条「什么时候该重取角色册」的纪律。
