...
**`panel/constraints.py::forbidden_entities()` 本身没有删**——R2（FUTURE_LEAK）和右栏
面板还在读它，这一刀只砍「把它渲染进 Writer prompt」这一步。`graph_section()` 函数
与「图谱事实只追加在用户消息尾部」这条性质（下面一、ADR 0010 D4）也没有删：
它是任何未来图谱事实的落地点，不是这一个块专属的脚手架，只是**今天没有任何东西
往里面填**，所以它现在恒返回 `""`。
