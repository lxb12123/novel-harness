...
摘要不进 decision_log——那是作者确认的事实账，摘要不算数，这条没变。
但「进写作 prompt 时要不要标『机器摘要，仅背景』」2026-08-26 已经改口：
作者随时看得见、随时能改，摆在那儿的内容按内容用，不按出身打折（ADR 0017 补记）。
同一章同一 prompt 只付一次调用，由 DB 唯一键兜底。
...
class ChapterSummaryStatus(BaseModel):
    ...
    ``has_text=True`` 且 ``summary is None`` = 有正文、**没生成过**（补它要花钱；
    2026-08-25 起没有手动入口，只等两个自动触发：保存之后 / 每 30 分钟扫描）；
...
def _address(payload_hash: str, revision: int) -> str:
    ...
    * 「撤回之后再生成一次」：同一章正文 = 同一份 prompt = 同一个哈希，同样撞键。
      ⚠️ **这一条 2026-08-25 起走不到了**：手动入口整条删了，撤回是终态——两个自动
      触发都不碰撤回过的章（见 `chapter_refresh._head_missing`）。**但这个键的形状
      没变**，所以这段病史照旧有效：它讲的是「同一份 prompt 撞同一个哈希」，
      而那件事在别的路径上还会再来一次。
