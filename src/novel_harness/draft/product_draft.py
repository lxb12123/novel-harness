...
def check_request(request: ChapterDraftRequest) -> None:
    ...
    今天唯一的调用方是 `draft_chapter()` 自己（模式二 agent 工具那条进程内直调路，
    `agent/drafting.py` → 这个模块）——它在函数最开头调一次，不在别处。

    ⚠️ **2026-08-26 之前 HTTP 壳（`/draft` 起草端点）在算约束之前也调一次**，为的是
    保住 422 的先后顺序（文风里写了禁令词和在场角色解析不了同时发生时，作者收到的
    仍然是文风那一句）。同一天 `write_rule` 从 `/draft` 的请求体上删掉（`ADR 0015`：
    文风改挂在对话上，由起草工具带进来），那条路从此再也造不出「文风也错、角色也错」
    的碰撞——`check_request` 唯一会拒的就是非空 `write_rule`，而这儿的 `write_rule`
    此后恒为空串。那次预调因此变成死代码，随 `1dec5af` 删了（原地留着一句注释说明，
    见 `api/app.py` 起草端点里的那句「这儿原来先调一次」）。**这不是说这个函数本身
    没用了**——`draft_chapter()` 自己那一次调用，理由和 HTTP 壳当年的理由是同一个，
    只是今天只剩它一条路会真的触发拒绝。

    ⚠️ 2026-08-25 之前它还验一个 `form`（PRODUCT / X0 / X1 / X2 四选一）并返回选中的那一臂。
    三臂随 M2 一起删了（见 `assemble.py` 的模块 docstring），**它现在没有返回值**——
    留着这个函数是因为文风那条校验跟 form 无关，仍然要在算约束之前跑。
    """
