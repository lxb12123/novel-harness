...
* 本模块是一次**应用层合成**：一头是库里的总结行，一头是图层的角色册。
...
class SummaryMention(BaseModel):
    ...
    **`node` 是 `NodeRef` 不是 `Node`**（§10.5 第 3 条）：`NodeProps` 是 `extra="allow"` 的，
    ...
    `model_dump_json()` 直接进浏览器。秘密的**显示名**是角色册本来就在渲染的东西
...
class NodeSummaryMentions(BaseModel):
    ...
    需要前者，而调用方手上往往只有一个 id（从活动日志、从角色册点过来的时候，
...
class _Roster:
    """一次扫描要用的角色册快照：可匹配的称呼 → 它指向谁，加上它的内容地址。"""
...
def paragraphs_mentioning(
    ...
    角色册和那条 alternation 归本模块（`_roster` + `INDEXED_LABELS`）。调用方按段调
    `mentions_in_text` 也能算出同样的答案，但那是**每段重建一次角色册、重编一次正则**
    ...
    if not roster.owner:
        return []
    # **只把要找的那几个节点的称呼编进 alternation**：整本角色册编进去会命中一堆跟这次
...
def ensure_index(conn: Connection, store: StoryGraph, project_id: str) -> None:
    """把这个项目的倒排表补到「跟当前的总结和当前的角色册都对得上」。

    ── 什么时候会真的干活 ────────────────────────────────────────────────

    索引是 `(总结的那一行, 角色册那一版)` 的纯函数，两个自变量都是内容寻址的：
    ...
    * **角色册变** —— `roster_hash` 变 ⇒ 全书索引一个都对不上 ⇒ 整本重扫。
...
def _rebuild(
    ...
    *,
    ...
    完全等价（同一段字 + 同一版角色册 ⇒ 同一批命中），代价是几百条走主键的 DELETE。
    """
...
def _chunks(items: Sequence[str], size: int = 400) -> Iterable[Sequence[str]]:
    """SQLite 的 `?` 上限（默认 999）：722 章一次全作废是真会发生的（改一次角色册）。"""
...
def mentions_in_text(
    ...
    """**这段字**里出现了角色册的哪些东西。库一次都不碰，一个语义判断都没有。
    ...
    自己另起一份角色册的话，反查会稳定地少命中，而少命中是静默的。
...
def chapters_after_mentioning(
    ...
    *,
    ...
    那个函数每调一次都要把整本角色册解析两遍（`_ensure` 一遍、`_nodes_by_id` 一遍）。
...
def appearance_counts(
    ...
    """`{node_id: 提到它的章数}` —— **一次 SQL，整份角色册一起算。**

    左栏角色册靠它排序（出场多的排上面）。作者的原话是「按出场频率排序」，
    ...
    角色册几十上百个节点，那样就是几十上百条 SQL 加几十上百次 `_ensure`。
