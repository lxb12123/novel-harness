...
**第三段在执行层**：`ChapterRefreshCoordinator.run` 只跑单上排了的那几支
...
def request_summary_backfill(
    ...
    *,
    ...
    ── ✅ 2026-08-27 维护者裁定：**这一档保留** ──────────────────────────────

    此前有过一次口头裁定「总结的触发我要的仅仅只要这两个」（保存之后 / 每 30 分钟
    扫描），手动那一整条（按钮 + `POST …/summary` 路由）照此删了。**本档字面上是
    第三个**，所以悬了一阵。

    维护者的裁定原话：**「在一定的权重范围内总结一定是要有的，所以这个保留。」**
    ——那次「只要两个」针对的是**作者按出来的**触发；本档和另外两个一样是系统自动的，
    而且它下的是同一种单、走同一个判据。**别再把它当成待裁的东西。**
