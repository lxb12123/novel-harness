...
class NodeUsage(BaseModel):
    ...
    抽取会**自动建人物**（ADR 0020 补记），而它会认错——真书上的实例是「袭人」
    （满篇「寒气袭人」）。**自动建 + 不能删 = 单向阀**，所以删除入口是那条裁定的配套。

    `alias` / `summary_mention` / `edge.src|dst` / `event_participant` /
    ...
    `api/characters.py::delete_node` 在删除的同一个事务里挂）。悄悄蒸发的东西
    没有变得可以接受——**只是防线从「事前拦住」搬到了「事后找得到、点得到」**。
    这个类型本身没有变化：它数的还是同一批东西，只是从「决定删不删得掉」
    降格成「删完之后告诉作者带走了什么」。
    ...
    一条边、一件事都没有，所以它删得掉且不会触发任何通知。今天这个差集仍然
    决定通知触不触发的边界完全一样——只是一个真的参与过情节的人现在也删得掉，
    差别只在于删完之后他参与过的那些事件会不会挂一条 `event_cast_changed`。
    """
