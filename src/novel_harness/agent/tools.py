...
**写前校准那两条（`calibrate_scene` / `seal_scene_brief`，ADR 0033）2026-09-12 砍了**
（[ADR 0047](../../../docs/adr/0047-drafting-is-assistant-curated-not-calibrated.md)）：
它们锁死的「要写什么」装不下作者的话，报告又大到助手读不到。现在 `draft_chapter`
直接收助手写的 `brief` + 它挑的 `materials`，写手那六格照旧后端固定装配。

**书内索引**（`index.py`，四层，越往下越贵；那份 docstring 是它的规格）：

| 工具 | 层 | 它回答什么 |
|---|---|---|
| `book_index`         | L0 | 全书章标题 + 角色册（秘密只给**显示名**） |
...
── 落盘：**表里没有一条工具动得了书**（ADR 0048，2026-09-12）──────────────────

ADR 0019 边界一原来的最后一条是「写正文必须作者确认，模型不能直接落盘」。
ADR 0021 一度推翻它（起草完直接写磁盘，不弹框）；ADR 0022 把机制拆成生成 / 落盘两个
动作（`draft_chapter` → `save_draft`）；2026-09-12 作者把落盘**拿回了自己手里**
（[ADR 0048](../../../docs/adr/0048-drafting-writes-the-chapter.md)）：稿子在写的过程中
流进他左边的正文编辑器、以未保存的样子放在那儿，他按「保存」才写进书——写盘走的是
他自己那条 `PUT …/chapters/{n}/text`（带 `draft_id`，候选表据此记「进书了」）。
`save_draft` 删了，`ChapterDesk.land` 也删了：**模型手上没有落盘这个动作**。
边界一最后一条因此又成立了，而且比原来更硬（`tests/test_agent_tools.py::
test_no_tool_takes_a_paragraph_of_prose` 钉着这一节）：

- **能写进磁盘的只有作者自己按的那一次保存。** 表里没有任何一条工具收一段正文，也没有
  任何一条工具写盘：模型不能拿一段自己编的（或者从别处抄来的）文本去盖某一章。
  这条以前靠「表里根本没有写工具」成立，现在靠**入参形状**（`draft_chapter` 收 `brief` /
  `materials`——那是给写手的话，不是正文；`read_draft` 只收编号）和「`ToolContext` 上
  没有写入面」两条一起成立，而它们都是可断言的。
...
|---|---|---|---|
| 角色册里没有 | `len(hits) == 0` | **别再试** | `index.UnknownCharacter` |
...
从第二次开始把清单附在拒绝后面（「你已经在角色册外面撞了 2 次」）。它是一个**只增不减
...
把人加进角色册，缓存一个「查不到」就是造一个看起来完全正常的错误答案）。
...
from .panels import (
    ...
    NotificationsArgs,
    ValidationRulesArgs,
    handle_character_card,
    handle_chapter_events,
...
class ConstraintsResult(BaseModel):
    """`scene_constraints` 的出参：这一章数出来的在场角色。

    **2026-08-31：`forbidden_entities` 字段删了**（未登场实体的禁写清单，
    ...
    """本章正文里被提到的角色册称呼（ADR 0018：在场是数出来的，不是作者填的）。"""
...
def _derived_cast(context: ToolContext, chapter: int) -> list[str]:
    """第 `chapter` 章正文里提到的角色册称呼（ADR 0018）。读不到正文就是空的。
...
def _derived_cast_from_text(
    ...
    """正文里提到的角色册称呼，正文由调用方一次读取后传入（ADR 0033 §8.4）。

    走的是 `mentioned_cast` 那条既有实现（和 R2 FUTURE_LEAK 同一条正则 alternation，
    一个语义判断都没有）。**不在这里重新数一遍**：第二份实现会和第一份漂移。

    ── ✅ 2026-08-27 维护者裁定：**8 个全报，保持现状** ──────────────────────

    裁定原话：**「那就指向八个人影响不大。」** 下面那段是这条裁定之前的病历，
    留着是因为它记着「当初为什么这么定」和「那个理由什么时候失效的」——
    **但『两侧都不是安全那一侧』这个僵局已经由维护者拍板破了，不要再当待裁项。**

    ── `expand_ambiguous=True` 的理由 2026-08-25 失效了，行为**没动** ─────────
    ...
    **行为一个字节没改**——2026-08-27 维护者裁定保持这样。下面这个问题当时没有好答案，
    裁定用的是「影响不大」这个产品判断，不是从两侧里选出了安全的那一侧：
...
def _scene_context(context: ToolContext, chapter: int) -> DraftContext:
    ...
    **歧义已经不从这道口子掉下去了**（2026-08-22）：`_derived_cast_from_text` 把
    ...
    只有角色册本身病了的那几种（两个节点重名、节点没有 canonical 别名行），
...
def _target_snapshot(context: ToolContext, chapter: int) -> TargetChapterSnapshot | None:
    """一次读取目标章当前正文（磁盘）+ 哈希。**全链唯一的一次读。** `None` = 这一章还没有正文。"""
    if context.root_path is None:
        raise ToolRefused(message("no_manuscript_root", context.language))
    text = read_chapter(Path(context.root_path), chapter)
    if text is None:
        return None
    return TargetChapterSnapshot(chapter=chapter, text=text, sha256=text_digest(text))
...
def _handle_character_state(
    ...
    # 合成一句「换个说法再试」等于由引擎亲口鼓励它去烧下一步，而角色册里没有的名字
    ...
    if node.label is not NodeLabel.CHARACTER:
        # 集合判断，不是语义判断：秘密 / 地点 / 物件也在角色册里，拿它们去查「状态」
...
def _handle_draft_chapter(args: DraftAsk, context: ToolContext) -> DraftResult:
    """起草一稿（ADR 0047）：在场从正文数、六格记忆后端装，助手给的只有 `brief` 和 `materials`。"""
    desk = _desk(context)
    ...
    with context.db_guard:
        # **一次读取目标章当前正文快照**：写手看的正文、候选 `base_sha256` 全用同一个对象，
        # 禁止各自重读磁盘。这一章还没有正文时（新章）快照是 `None`，在场按空正文算。
...
class ToolSpec:
    ...
    **空 = 没写**，`tool_label()` 那时给一句不认字的通用说法，**绝不原样回吐工具名**
    ...
    今天只有 `draft_chapter` 两条都满足：它不动书（稿子流进作者的编辑器，落盘是他自己
    按保存，ADR 0048），而它是表里唯一一个每次要跑几十秒的（一次真的模型调用）。
...
TOOL_TABLE: Final[tuple[ToolSpec, ...]] = (
    ...
    ToolSpec(
        ...
        description=(
            ...
            "上一章结尾——**这些不用你抄进来**。**它看不到这一章现在的正文**（给了它就照抄），"
            "所以这是整章从头写；要保留其中几段原样、只改别的，用 revise_passage。\n"
    ...
    #
    # 作者的原话：「我要求把这几个功能给他补进去，就是能读这里面的功能……每个按钮功能
    # 里面的每个内容」。四条全是读，读的是面板自己那条读法；边界一那个问题的答案
    # 写在 `panels.py` 顶上（出参只有 NodeRef 和纯量，角色卡那几行字和进写作提示的是
    # 同一份收窄视图）。
    ToolSpec(
        name="character_card",
        description=(
...
class TurnMemo:
    ...
    **但它一步都省不下来**：一步 = 一次模型调用 + 它这一次要的那批工具
    ...
    第二次查询会拿一句「角色册里没有」去盖住一个已经有了的人，**而它和一次正常的返回
...
def _already_missed(names: Sequence[str], language: DraftLanguage) -> str:
    ...
    第一次撞空不说这句（`len < 2`）：那一次它还不知道这本书的角色册有多严，
...
def dispatch(
    ...
    4. **工具自己拒绝** —— 角色册里没这个名字、称呼有歧义、能力没接线。
...
class BatchRunner:
    ...
    起草不动书（ADR 0022 拆出去、ADR 0048 交回作者自己按保存），`draft_chapter` 没有
    副作用——**一批三稿可以同时写**，作者盯着转圈的两三分钟压回一分钟。
