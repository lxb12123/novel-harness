...
class TurnLimits:
    ...
    **数两个数，缺一个就有一条绕过去的路**（两个都用这一个阈值）：
    ...
    """这一轮里**撞空过的不同称呼**有几个算「它在角色册外面打转」（`tools.TurnMemo`）。
    ...
    几个不同的称呼被证明不在角色册上**。差别在于后者是一个**不会因为别的查询成功而改变
    的事实**——角色册这一轮不会长出新名字（作者在另一个窗口里加人是下一轮的事，
    而下一轮这个记性是空的）。

    **它数的是「不同的称呼」，不是「撞空的次数」**：同一个名字反复查是「原地打转」，
    上面那两个数已经罩住了；这一个专管「换个名字接着查」，也就是实测的那个形态。

    默认 3：查一个查不到、直接往下写是正常的；连着三个不同的名字都不在角色册上，
    ...
    while steps < limits.max_steps:
        ...
        for position, call in enumerate(calls):
            ...
            if (
                ...
                # 角色册里没有的名字不会因为别的查询成功就变得查得到，所以这个计数只增。
