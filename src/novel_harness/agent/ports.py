...
class DraftDesk(Protocol):
    """起草这件事的**全部**接线口：生成 / 按 id 读回。**这一层不动书。**
    ...
    ADR 0022 曾有第三个动作 `land`（模型决定、不问作者、sha 闸拒绝覆盖他更晚的改动）。
    2026-09-12 起稿子直接流进作者左边的正文编辑器、**以未保存的样子**放在那儿，
    他按「保存」才写进书——写盘走的是他自己那条 `PUT …/chapters/{n}/text`
    （带 `draft_id`，后端据此在候选表上记「这一稿进书了」）。模型手上没有落盘这个动作，
    也就没有「要不要存」这个问题可问。

    ── 为什么是一个端口而不是两个字段 ────────────────────────────────────

    两件事握着**同一批东西**（那条连接、那张候选表）。拆成两个注入口，装配层就能只接
    其中一个，而「生成得了、读不回来」这种半接线状态没有任何东西会报错。
    """
