"""本章正文里提到了角色册中的哪些称呼 —— 把「谁在场」从**作者的输入**变成**引擎的推导**。

── ⚠️ 一条边界，别越（2026-08-25 裁定）────────────────────────────────────

维护者的原话是「这个大模型分析就好，不要用正则」。**那句话管的是「谁在这一章」，
不管规则**：

    抽取那一侧   本来就是模型判断        ← 改成信它，没问题
                （`extract/service.py` 从这一天起认不出就建人物，ADR 0020 补记）
    规则那一侧   R2/R3 必须是字符串匹配   ← **不许改**，撞 ADR 0005

本模块产出的东西**是给面板和上下文用的，不是给规则用的**：R2 `future_leak` 和 R3
`dead_speaks` 各自调 `text/mentions.py`，一次都不经过这儿。所以「这一章提到了谁」
将来真要改成信模型，不撞那条铁律——ADR 0005 当年测算过，语义规则 5~20 条误报/章，
作者会关掉功能。**要动之前先确认你动的是哪一侧。**
...
信里写到的名字都会进来）。那是这一层的事实，只是它不再自动等于「安全」。
...
def mentioned_cast(
    ...
    *,
    ...
    Notes:
        ...
        **只收 Character。** 角色册里还有地点、势力、物件——`青云城主府` 出现在正文里
        是常事，但把它塞进 cast 会让【在场】那一块出现一个不是人的名字。
