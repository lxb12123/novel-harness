...
> **2026-08-25：100% 完成。** Task 1–11 全部落地，产物在 `src/novel_harness/calibration/`
> （`calibrate.py` / `seal.py` / `render.py` / `repair.py` / `handoff.py` / `freshness.py` /
> `visibility.py` / `store.py` / `ports.py` / `models.py`）。
>
> 「十三、完成判据」十条里,第 1 条和第 3 条的措辞已被 ADR 0039 改动:
> 秘密下线之后不再有「不安全的记忆」这一类,`UnknownCast` 那个总开关连同它的对象一起没了。
> 其余八条成立。
