...
| POST | `/projects/{pid}/chapters/{n}/draft` | `scene_view` → `build_product_context` → `assemble_continuation`；**外加 `track.build_track`（2026-08-23，不进 prompt）** | `{experimental, note, text, memory, track, length, …}`·**这一行 2026-08-02 起就不是 501 了**（修正案 7 实验开放）；`memory` 是记忆层回执，**零带着理由**（装了几份档案/事件/总结、哪几章缺总结）；`previous_tail` 的截断长度**分档**——PRODUCT 从模型窗口倒推（`product_tail_limit()`，2026-08-10 起），点名 X0/X1/X2 则原样拿冻结的 800（那是考卷，见 ADR 0019 边界五）。**`track` 是它的反面**：作者改的不是最后一章时，后面哪几章的总结跟这一段相关（`track.py`，零模型调用）——`memory` 说「prompt 里装了什么」，`track` 说「查到了什么、而且一个字都没进 prompt」（后面章节的总结里可能有这一章还不该知道的事，`tests/test_track_isolation.py` 两道钉着）。入参多一位 `following_text`（光标后面那截同章正文）：改旧章时它渲染成【下文】块并写死「别重写、要能接上」，在最后一章上则一个字节都不进 prompt。**⚠️ 2026-08-26：这条路只剩行内续写了。** 「起草一整章」那个入口删了——它 2026-08-14 起就零调用方（`useDraft` hook 那天删的），而模式二的 `draft_chapter` 工具是**进程内直调**`draft/product_draft.py`，不发 HTTP。请求体上的 `mode` / `goal` 跟着走了（`goal` 是 ADR 0015 D3 点名的泄漏入口，现在**形状上就没有这一位**）。`write_rule` 也一起走了（**只有整章那一支读它**，前端一处不发；作者的文风今天挂在**对话**上，由起草工具带进去，`WRITE_RULE_FORBIDDEN_HINTS` 那张禁词网照旧在 `product_draft.check_request()` 里拦着）。**这三位是拒收不是忽略**：`mode="chapter"` → 422 并说清入口去哪了，`goal` / `write_rule` 非空 → 422——忽略它们等于「发一份写整章的请求、拿回一段续写」和「你以为模型读过你写的那句话」，而本仓库的纪律是**静默丢掉才是坏的**。`mode="continuation"`（旧前端发的那份）、`form`、任何别的键**照旧忽略**，2026-08-25「不加 `extra="forbid"`，别让老客户端炸掉」那条裁定一个字没动——它讲的是空操作键，罩不住会换掉产品的那一位。`cast` 留着，它是 D4 明写的「写得更准的奖励」。同一刀把 `plan_call` 的 `ReasoningEffort.HIGH` 改成了 `OFF`——那个 HIGH 是当年为整章起草写的，ADR 0015 把续写接进来之后它压在续写头上，**在作者自建（未登记）的端点上是当场 `CapabilityError` ⇒ 每次续写恒 422**，而措辞把作者支去重填一份没问题的配置。论证只有一份，在 `agent/drafting.py::AGENT_DRAFT_REASONING` 的 docstring 里 | 🟢 |
...
> 以及他中途说的话这一轮没读到（`receiptNotes`，零不写）。**上下文裁掉了什么不上屏**
> （2026-09-12 作者裁定：「系统内部的提醒出来给人看干吗」——那几个数留在 `receipt.context`
> 给维护者，屏幕上只有作者自己的事）。
>
...
│  │                                无关。（这儿原来接着一句「右栏别的格全靠人」——那道
│  │                                「角色册空就把检验规则 / 事件整格换成『先去加人』」的闸
│  │                                2026-09-13 拆了：检验规则查的是字面，「事件」的工具栏上
│  │                                就是「分析本章」，正是把人物整理进角色册的入口，闸门把它
│  │                                一起藏了。现在每一格空着时各说各的空态，都写明它怎么填满。）
