...
- 状态：~~已接受~~ **已退役**（2026-09-12，[ADR 0047](0047-drafting-is-assistant-curated-not-calibrated.md)：整条写前数据链砍掉，起草改成助手分析意图 / 检索 / 对照规矩后直接给写手「要写什么」和「补的资料」）
- 日期：2026-08-17
...
## 补记（2026-09-12）：模型拿到的不是整份 `CalibrationReport`

第 1 步「产出 `CalibrationReport`」在**库里**仍然成立；**交给模型的是按预算收过的一份**
（`agent/tools.py::CalibrationView`）。真书实测的事故：报告把第 1–N 章的全部摘要都装成
事实（`calibrate.py` 第 5 段），158 章的书上一份报告 232 KB，比一次工具返回的天花板
（`return_units`）大七倍——投影层只能整条收起（`PRUNED_RESULT`），模型**从没读到过它**，
连报告的 `id` 都没见过：猜了一个 `insp_c1b0f` 去封存、去起草，两次被拒，又校准三遍，
八步用完（`STEP_LIMIT`），一稿没写；作者看到的是「它来回查了太多次」。

收法：在哪 / 死没死 / 登场没 / 关系先留全；剩下的预算分给事件 / 状态 / 摘要，三份都从
离第 N 章**最远**的那一端丢（`_calibration_view`），丢了多少写进 `notes`。`id` 排在最前面。
封存和起草照旧按 `id` 读库里完整的那份，Writer 拿到的东西一个字没变。

同日顺手修的第二处：报告里事件的上限 `EVENT_CAP = 40` 原来切的是**最早**的 40 件
（按章号升序取头），给第 158 章校准时报告里全是第 1–4 章的事；改成留最近的 40 件。
