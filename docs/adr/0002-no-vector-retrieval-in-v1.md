...
## 补记（2026-08-25）：(b) 的例子没了，结论**还在**，但它现在只靠 (a) 和 (c) 撑着

维护者裁定秘密整套功能下线（[ADR 0039](0039-secrets-offline.md)）。**正文一个字未改**，
下面是这个决定对本 ADR 论证的影响，照实记，不找补。

**(b) 那一节整节失效了。** 它举的例子是「第 152 章时谁不知道血脉秘密」，论证是
「答案不在任何一段原文里，任何 RAG 都检索不到它，因为它不存在于文本中，只存在于聚合里」。
D 分区（Character Knowledge Boundaries）今天不存在，那个集合差集**算不出来也没人要**。

**由此塌掉的推论：**

> 「v1 不需要跑『向量 RAG』对照组来证明自己。普通 RAG 在结构上就做不到这件事。」

这半句的支点就是那个例子。**今天没有一条能力是「RAG 在结构上做不到」的**：
剩下的确定性查询（`state_at` 的闭开区间、`forbidden_entities` 的首现章比较、
mention 倒排）都是**在文本里查得到线索**的东西——RAG 做得好不好是另一回事，
但「结构上不可能」这句话不再成立。

**所以：这一条不再是「不欠一个对照组」的理由。** 真要主张 v1 的检索比向量强，
今天得靠实测，不能靠这段推理。

**(a) 和 (c) 没有被碰。** (a) 数的是 10 个分区里几个需要 ANN——D 分区消失只让分子分母
一起减一，「大多数分区是确定性查询」的结论方向不变；(c) 说的是向量的三件第一天成本
（增量重建、`style_dense` / `interaction_dense` 是伪信号、中文 embedding 的实际质量），
跟秘密毫无关系。

**决策本身不动**：v1 仍然不做向量检索。变的是它的**论证强度**——原来是三条腿，
现在是两条。「加向量的触发条件」那一节照旧有效。
