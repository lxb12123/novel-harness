...
> **补记（2026-08-25）：撤回从此是终态。**
>
> 这一条原文只说了「撤回不是删行」——**它没说撤回之后还能不能再拿回一份**。
> 当时的答案是「能：作者自己点『生成总结』」，那条退路写在
> `POST …/chapters/{n}/summary` 的 docstring 里。
>
> **那条路 2026-08-25 整条删掉了**（按钮 + 路由 + `create_manual_attempt`）：
> 总结的触发只剩两个，**都是系统自动的**——保存之后、每 30 分钟扫描。
> 而这两个都按设计**不碰撤回过的章**（「他删一次，系统别买回来一次」，
> 那条纪律 2026-08-21 踩过一次坑之后有两层守卫钉着），
> 实测「撤回之后又改正文」也仍然不买回来。
>
> **所以撤回之后系统永远不会再买一份。** 作者剩下的回头路只有一条：
> 自己写一段（`PATCH …/summary`，不花钱）。
>
> **这是裁定，不是洞。** 下一个人读到「撤回之后再也补不回来」时的正确动作**不是**
> 去把自动补缺放开——那会让「他删一次系统买回来一次」原样回来。
> 要改的话得先把「谁按的、谁付钱」那条重新想一遍。
