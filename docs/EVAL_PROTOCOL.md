...
> 📌 **本文件有五份修正案，动手之前先读**（2026-07-27 / 07-30 / 08-01）。
...
> [`修正案 5`](EVAL_PROTOCOL_AMENDMENT_5.md)（2026-08-01） —— ⑧ 将 M2 输出冻结为中文
> 2,000–3,000 非空白 Unicode code point；225 指 final cell，每份不足 2,000 时最多一次固定续写，
> 所以成功基准轮是 225–450 次 transport call；并冻结通用 provider 的 high reasoning 能力预检、
> token 预算、streaming 与 attempt/final/length-invalid 证据形状。
> **下面的冻结内容一字未改**（`git show 0393088:docs/EVAL_PROTOCOL.md` 可取回原文，本次为纯增量）；
> 五份修正案仍全部属于预注册：1–4 写下时 `runs/` 与 `synth/` 都不存在；第 5 份晚于
> `synth/`，但写下时 `runs/` 仍不存在、一次真实模型推理都没跑过。
