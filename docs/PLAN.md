...
#### 【改 5】全书自动抽取 → 别名花名册 + 增量。改了第 22 节第 2 项和第 17 节 Import 页。
...
**改成：** 导入 = 章节切分 + 别名花名册自动抽取 + 作者做**聚类确认**（不是逐条确认）。花名册是唯一值得前置人力的东西——量小（300 章的小说约 50–200 个实体）、作者几秒就能判、所有下游都以它为键，而且 GenWebNovel 显示 PER 恰好是抽得最准的一类，错误集中在别名归并，正好是人工最省力的地方。确认形态必须是对聚类的（「李明在 47 章里被叫作明哥 —— 全部接受？」= 1 次点击换 47 条），不是对条目的。半小时，不是 130 小时。
...
| 别名花名册 | 50–200 个实体 | **自动抽 + 作者聚类确认** | 30 分钟 |
...
| 2 | 初始图谱抽取与作者校对 | **改：只做别名花名册 + 聚类确认** | 增量抽取 → M4 | 130 小时墙。见【改 5】。 |
...
| 5 | serious | 冷启动墙（真书 60+ 人物 / 200+ 别名，设定在作者脑子里） | **修：** ① 秘密量级 5–50 条不是 5000 条（§3.4）；② 花名册自动抽 + 聚类确认（PER F1 76.60，归并是人工最省力处）；③ cast 由 mention 自动建议；④ **价值曲线从 t=0**：手填一个秘密 + 一条 KNOWS，面板就开始工作。⑤ M1 的 done_when 改成真书数字（见 §7）。 |
...
| **M3** | 规则检测（R2/R3/R4[/R5]） | **2 周** | 11 | **双边门槛**：真书连续 20 章，**误报 < 1 条/章**（人工判定，定义已预注册）**且**合成小册子上真阳性 ≥ 22/25。任一不达标不许进 M4 |
...
| 时间 | 版本 | 内容 | 渠道 |
|---|---|---|---|
| **第 5 周** | v0.1 pre-alpha | 面板 + 花名册 + demo 书 | GitHub only，V2EX 一帖，不上 HN |
