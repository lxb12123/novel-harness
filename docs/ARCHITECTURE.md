...
**M0 + M1 + M1.5 已落地；M2 已由维护者裁定通过（修正案 9 / ADR 0009，非数据裁决）；M3 双边门槛已过、M4 事件记忆切片已落地并通过真书三章接受度验收**（抽取 → 提案/被动确认 → 作者审阅 → 安全事件上下文全闭环；111935 第 1–3 章：26 条有效事件、冲突 0 条/章、接受率 100%，2249 个 pytest + 427 个 vitest 全绿，`.sql` 和前端产物都在 wheel 里）：
...
>
> 守卫钉住的是**能在运行时数出来**的那些（59 条路由 / 58 条 /api / 1 条 501 stub /
> 19 个错误映射 / 18 个子命令 / 24 张表 / 56 个端点 / `ALL_CHECKS` 3），
> 改错必红、**删掉也必红**（不静默 skip）。
...
> 所以「2249」和「427」仍然只靠人手改，改代码后请顺手跑一次 `uv run pytest -q` 更新这一处。
...
api/{app,deps,activity,autopilot,chat,extraction,review}.py
                                                ← M1.5 FastAPI 壳：59 条路由 + 19 个错误映射
                                                  （58 条 /api + 1 条 `GET /`；其中 1 条是 501 stub；
...
frontend/src/                                   ← React 工作台：44 个非测试手写源文件、8910 行 TS/TSX（2771 行测试）
                                                  ...
                                                  三种拒绝三句话，**409 绝不静默重试**））
frontend/src/__fixtures__/api.json              ← 从真 app dump 的 56 个端点出参（契约测试两头共用）
...
agent/{ports,index,tools,loop,store,model,drafting,candidates}.py
   ...
   **前端一个调用方都还没有**（那是 3.5：中栏对半分，左正文右助手）。
   ...
   - ~~**按「停」中断不了一次正在跑的起草。**~~ —— **2026-08-12 已补。**
     ...
     **它换来的三条新欠账，写在这儿免得下一个人当成 bug：**
     ...
     绝不编一个），但**底栏那个 `COALESCE(…,0)` 会把它显示成「入 0 / 出 0 token」**
     （那条已知病因此变得更常见）。要挽回得先给那几条路由登记 stream usage 的证据。
