...
> 19 个错误映射 / 17 个子命令 / 46 张表 / 87 个端点 / `ALL_CHECKS` 2），
> 改错必红、**删掉也必红**（不静默 skip）。
...
migrations/{001_init,002_m4_events,003_proposal_audit_recovery,004_chapter_summary,005_fact_edit,
            ...
            012_chat_notice,013_summary_edit,014_summary_mentions}.sql（46 张表）
...
frontend/src/__fixtures__/api.json              ← 从真 app dump 的 87 个端点出参（契约测试两头共用）
