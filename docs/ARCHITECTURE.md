...
**M0 + M1 + M1.5 已落地；M2 已由维护者裁定通过（修正案 9 / ADR 0009，非数据裁决）；M3 双边门槛已过、M4 正在实现（尚未完成）**（1134 个 pytest + 33 个 vitest 全绿，`.sql` 和前端产物都在 wheel 里）：
...
> 所以「1134」和「33」仍然只靠人手改，改代码后请顺手跑一次 `uv run pytest -q` 更新这一处。
