...
**M0 + M1 + M1.5 已落地；M2 的修正案 5/6/7 / ADR 0011 已预注册，配套实现与 endpoint/profile 已冻结，剩第一轮有效真模型运行**（1026 个 pytest + 33 个 vitest 全绿，`.sql` 和前端产物都在 wheel 里）：
...
> 所以「1026」和「33」仍然只靠人手改，改代码后请顺手跑一次 `uv run pytest -q` 更新这一处。
...
### M2 的当前形状：修正案 5/6 / ADR 0011 已冻结，实现与 endpoint/profile 已落地，剩第一轮有效真模型
...
（1026 个 pytest / 33 个 vitest 全绿，见下）；2026-08-02 又补了两件事：DeepSeek 共享输出预留
