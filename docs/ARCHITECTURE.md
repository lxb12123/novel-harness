...
**M0 + M1 + M1.5 已落地；M2 的起草线在用、评估那一半已退役；M3 双边门槛已过、M4 事件记忆切片已落地并通过真书三章接受度验收**（抽取 → 提案/被动确认 → 作者审阅 → 安全事件上下文全闭环；111935 第 1–3 章：26 条有效事件、冲突 0 条/章、接受率 100%，2466 个 pytest + 639 个 vitest，`.sql` 和前端产物都在 wheel 里）：
...
>
> 守卫钉住的是**能在运行时数出来**的那些（85 条路由 / 84 条 /api / 1 条 501 stub /
> 19 个错误映射 / 48 张表 / 84 个端点 / `ALL_CHECKS` 2），
> 改错必红、**删掉也必红**（不静默 skip）。
...
summary_index.py                                ← **每一段章节总结 = 一个可反查的记忆点**（2026-08-13）。
                                                  ...
                                                  **三条写路径一个字都不用改，也没得忘**；
                                                  ② 花名册那一版——`roster_hash` 一变，全书索引全对不上，
...
api/{app,deps,launch,activity,background_runtime,chat,extraction,manuscript,notifications,reconcile,review,validation}.py
                                                ← M1.5 FastAPI 壳：85 条路由 + 19 个错误映射
                                                  （84 条 /api + 1 条 `GET /`；其中 1 条是 501 stub；
...
frontend/src/__fixtures__/api.json              ← 从真 app dump 的 84 个端点出参（契约测试两头共用）
