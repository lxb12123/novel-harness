...
**第二条泳道**每天量一遍：它走 HTTP 建书、import 一本 3 章的 fixture、`declare/death`
（作者只给一句从正文复制的话）→ 断言响应里的 `valid_from_chapter = 3`。两条泳道各证
一件事，谁都不替代谁。

⚠️ **别把守卫扫不到这里读成「这里可以查图」。** 那道守卫拦的是「第二份时态过滤」，
而本文件一行读查询都没有：它只往图里填作者本该亲手声明的东西，读那一侧全部由
Web 工作台的 API 走 StoryGraph 完成——**心跳要量的正是那条读路径，
...
（`EdgeSpec.valid_from_chapter` 一直是公开的、`ge=1` 的字段）。**而 Web 的表单至今没有、
也不许有一个填章号的框。** 这两件事不矛盾：一个是已确认状态的 fixture，一个是作者的输入面。
"""
