...
（逐项状态见 [ARCHITECTURE 的「当前状态」](docs/ARCHITECTURE.md#当前状态)。协议本体之外还有七份修正案，
`EVAL_PROTOCOL_AMENDMENT_{1,2,3,4,5,6,7}.md`；1–4 早于 `synth/`，第 5 份晚于 `synth/`，
前五份早于任何真实推理，第 6 份（超长 ≤100 宽容）在四轮 INVALID 之后、任何有效轮之前，
第 7 份（起草先行开放，实验状态）由维护者裁定；全部另开文件、不覆盖正文。）
