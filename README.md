...
> **尚未验证并向作者开放的**：AI 起草线（M2 的代码已合拢——三臂、判分、runner、`nh gate` 都在——**但一次真模型都还没跑过**）；**还没有的**：抽取（M4）。
...
| **M2** | 合成小册子 + 起草 + **kill-gate** ← 最早的证伪点 | 🚧 **进行中**（代码已合拢，等第一轮真模型，见下） |
...
M2 的链**已经合拢，但还没通电**。判分侧（泄漏检测 = 纯集合判断、精确 McNemar + Holm、
