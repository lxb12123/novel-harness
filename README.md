...
「看到结果再定及格线」在结构上就做不到。
**剩下的唯一一件事是接上真模型跑第一轮**（要一个 endpoint 和一笔预算）。
在跑完之前，README 里不会出现任何泄漏率数字，[ADR 0009](docs/adr/) 也不会写——
协议 §8 定死它「跑完写」，提前写正是预注册要防的那件事。
（逐项状态见 [ARCHITECTURE 的「当前状态」](docs/ARCHITECTURE.md#当前状态)。协议本体之外还有四份修正案，
`EVAL_PROTOCOL_AMENDMENT_{1,2,3,4}.md`，都在跑出任何结果之前写下、都另开文件不覆盖正文。）
