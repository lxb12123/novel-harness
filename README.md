...
> ⚠️ **Pre-alpha。** 已经能跑的是两条线：**终端里的**——SQLite 数据层 + 时态图层（`state_at` / supersede）、认知边界面板、声明层（敲引语，系统算章号）、R2/R3 一致性规则、切章，出口是终端里的 `nh`；**浏览器里的**——FastAPI 壳 + React 工作台（三栏 + 右栏 7 个 tab + CodeMirror 6 + 局部关系图）。pytest + vitest 全绿（前端吃的 fixture 是从真后端 dump 的，出参一改两头都红）；具体条数见 [ARCHITECTURE 的「当前状态」](docs/ARCHITECTURE.md#当前状态)——**那儿是全仓唯一一处写这些数字的地方**，本文件不再抄。CI 里写了 `uvx` 装机路径的三级验证（含「装出来的包里真的有工作台」），但**这个仓库还没有远端，CI 一次都没跑过**——目前全部只在维护者本机手跑过。
> **尚未验证并向作者开放的**：AI 起草线（kill-gate 225 格裁决尚未有真模型有效结果）；M4 事件记忆切片已落地（抽取 → 提案/被动确认 → 作者审阅 → 安全事件上下文），真书三章接受度验收待跑。
...
**今天这条对作者仍是自动成立的**：`draft/` 三块里没有一块会自己发起调用（`assemble.py` 只拼字符串，
`provider.py` 要别人递 config 才动），HTTP 路由里没有一条会调模型（会调的那 5 条今天全是 501 stub），
不配置任何 key 时整个工具完整可用。
**唯一一条真会联网的路径是 `nh gate`**——它是维护者跑 kill-gate 用的，作者不会敲到它，
而且没有 config 它会直接判死（`config=None` 是显式错误，不会现读环境变量偷偷跑起来）。
...
| **M2** | 合成小册子 + 起草 + **kill-gate** ← 最早的证伪点 | ✅ 维护者裁定通过（修正案 9 / ADR 0009，非数据裁决）；真模型 225 格运行待做 |
| M3 | 一致性规则 R2 / R3 | ✅ 双边门槛已过（维护者判定：真书连续 20 章误报达标 + 合成 25/25；R4 按 [ADR 0027](docs/adr/0027-scene-blocks-cut.md)、R5 按 ADR 0014 砍掉——**那 25 道题一道都不属于它们**） |
| M4 | 增量抽取 + 三层图谱 + exception-driven | 🚧 事件记忆切片已落地（抽取→审阅→被动确认→事件上下文）；真书三章接受度验收待跑 |
...
M2 的旧短输出链已存在，但**修正案 5 的链还没合拢，也没通电**。判分侧（泄漏检测 = 纯集合判断、精确 McNemar + Holm、
**预注册裁决表 `decide()`**、反混淆 `confound_lint`）、被判侧（三臂本体 `draft/assemble.py`）、
产出侧（`eval/runner.py` + `nh gate` + 合成小册子 `synth/`）都已落地并有测试。
建造顺序是**判分器先于被判者**——先有卷子和判分口径，再有被判的东西，
「看到结果再定及格线」在结构上就做不到。
修正案 5 与 ADR 0011 已先冻结中英文长度、通用 provider 能力与一次续写规则；对应实现和离线
验证完成后，才会冻结实际 endpoint/profile 并跑第一轮。
在跑完之前，README 里不会出现任何泄漏率数字，[ADR 0009](docs/adr/) 也不会写——
协议 §8 定死它「跑完写」，提前写正是预注册要防的那件事。
（逐项状态见 [ARCHITECTURE 的「当前状态」](docs/ARCHITECTURE.md#当前状态)。协议本体之外还有九份修正案，
`EVAL_PROTOCOL_AMENDMENT_{1,2,3,4,5,6,7,8,9}.md`；1–4 早于 `synth/`，第 5 份晚于 `synth/`，
前五份早于任何真实推理，第 6 份（超长 ≤100 宽容）在四轮 INVALID 之后、任何有效轮之前，
第 7 份（起草先行开放，实验状态）由维护者裁定，第 8 份（长度 ±10% 降权）在七轮 INVALID 之后；
第 9 份（维护者裁定 M2 通过，非数据裁决）在第九轮进行中裁定；全部另开文件、不覆盖正文。）
