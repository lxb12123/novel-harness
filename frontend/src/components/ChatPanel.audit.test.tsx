...
describe("秘密内容：带毒的会话详情", () => {
  ...
  it("**面板不去别处捞原文** —— 它打的每一条都在 `/chats` 底下", async () => {
    ...
    // 一轮跑完要让正文那一侧失效重取，但**失效不是取**：这块屏幕自己一条都不该去打。
    // 多出来的任何一条都要在这儿解释清楚。（「本章 N 稿」那条列表路由 2026-09-12 随
    // 并排页一起撤了；`/api/settings` 是 2026-09-13 加的——输入框上面那句「先连接模型」
    // 要知道模型服务配好了没有，读的是全局那份缓存，一次、不带任何正文。）
    expect(stray).toEqual(["/api/settings"]);
    // 而**一稿的全文一次都没取**：那一章正文只在作者亲手按「放入编辑器」时才取。
