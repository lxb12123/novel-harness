...
export function StateCards({ states }: { states?: StateSnapshot[] }) {
  if (!states) return <div className="empty">正在读…</div>;
  if (states.length === 0) return <div className="empty">无在场角色。</div>;
  return (
    <div>
      {states.map((s) => (
  ...
  return (
    <div className="lookback">
      <div className="lookback-head">
        <span className="lookback-badge">上一章</span>
        <b>正在看第 {prev} 章结束时</b>
        <span className="spacer" />
        <button onClick={() => onLookBack(false)}>回到第 {chapter} 章</button>
      </div>
